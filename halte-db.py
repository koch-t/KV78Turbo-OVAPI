...
def queryTimingPoints(environ, start_response):
    ...
    if 'town' in params and 'timingpointname' in params:
   	    ...
   	    if 'destinations' in params and 'accessibility' in params:
                 ...
                 cur.execute("SELECT timingpointtown, timingpointname, name,x.timingpointcode,stopareacode,kv55,kv78turbo,arriva55,destinationname50,lineplanningnumber,linepublicnumber,motorisch,visueel,x.latitude,x.longitude from  (SELECT latitude,longitude, timingpointtown,timingpointname,name,t.timingpointcode,stopareacode, kv55, kv78turbo, arriva55,destinationname50,lineplanningnumber,linepublicnumber, the_geom FROM timingpoint as t,destinationuserstop as d where d.timingpointcode = t.timingpointcode and destinationcode is not null) as x left join haltescan as h on (h.timingpointcode = x.timingpointcode) ORDER by ST_Distance(the_geom, st_setsrid(st_makepoint(%s, %s),4326)) LIMIT %s;", [longitude,latitude,limit])
